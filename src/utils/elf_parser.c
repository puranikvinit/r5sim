...
void extract_instructions(const char* elf_file_path, const char* output_file_path) {
    ...
    for (int i = 0; i < elf_header.e_shnum; i++) {
        if (section_headers[i].sh_type == SHT_PROGBITS && section_headers[i].sh_flags & SHF_EXECINSTR) {
            ...
            for (size_t j = 0; j < section_headers[i].sh_size; j += 4) {
                ...
                instruction = __builtin_bswap32(instruction); // Convert from little-endian
    ...
    printf("Extracted instructions written to %s\n", output_file_path);
    fclose(output_file);
    fclose(elf_file);
}
...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        fprintf(stderr, "Usage: %s <input_elf_file> <output_hex_file>\n", argv[0]);
        return 1;
    }
    extract_instructions(argv[1], argv[2]);
    return 0;
}